...
service=FastAPI(title="CA Address Lookup Service")

MAX_RETRIES=3
RETRY_DELAY=2
TIMEOUT=60

EXTERNAL_API_URL = "http://core-services-api.data-axle.com/v1/nua/microservices/ca_address_lookup"
...
class CA_Address_Lookup(BaseModel):
    header:Header
    request:Union[Dict[str,Any],List[Dict[str,Any]]]

@service.post("/process")
def process_ca_address_lookup(data:CA_Address_Lookup,request:Request):
    ...
    for rec in canonical_records:
        try:
            ...
            rec["services"]["ca_address_lookup"]={
                "status":"FAILED",
    ...
    if not requests_payload:
        return {
            "status":"CA_ADDRESS_COMPLETED",
            "canonical_records":canonical_records,
            "job_summary":{
                "service":"ca_address_lookup",
                "success_count":0,
    ...
    if MOCK_MODE:
        ...
        logger.info("Using Mock Mode for ca_address_lookup")
        # Create mock response
        mock_response = {
            "output": {
                "standardized_address": {
                    "address1": requests_payload[0]["address1"] if requests_payload else "123 Mock Street",
                    "city": "Mock City",
                    "province": "ON",
                    "postal": "M1M 1M1"
                },
            ...
            try:
                ...
                response=requests.post(
                    ...
                    headers={
                        ...
                        'User-Agent': 'CA_Address_Lookup/1.0'
                    }
                )
    ...
    for rec,result in zip(canonical_records,api_results):
        ...
        if "output" in result:
            ...
            rec.setdefault("meta",{})
            rec["meta"]["status"]="CA_ADDRESS_LOOKUP_COMPLETED"
        else:
            failure_count+=1
            rec["services"]["ca_address_lookup"]={
                "status":"FAILED",
